(function ($) {
    ...
    // Parallax layers 
    var layer_selectors = [
		".media_container .overlay",
		".whatcanido_container .overlay",
    ];

    layer_selectors.forEach(function(selector) {
      if ($(selector).length) {
        var layer = $(selector).get(0);
        var parallaxInstance = new Parallax(layer);
      }
    });
